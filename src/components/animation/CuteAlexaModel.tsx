...
export function CuteAlexaModel({
  ...
  const cpuRingRef = useRef<THREE.Mesh>(null);
  const centralCpuRingRef = useRef<THREE.Mesh>(null);
  ...
  useFrame((state) => {
    ...
    // Apply scroll-driven camera and rotation values
    if (animValuesRef && animValuesRef.current) {
      const vals = animValuesRef.current;
      if (robotGroupRef.current) {
        robotGroupRef.current.rotation.y = vals.robotRotationY;
      }
      if (cpuRingRef.current) {
        cpuRingRef.current.rotation.z += 0.04;
      }
      if (centralCpuRingRef.current) {
        centralCpuRingRef.current.rotation.z += 0.04;
      }
      state.camera.position.set(vals.cameraX, vals.cameraY, vals.cameraZ);
      state.camera.lookAt(new THREE.Vector3(vals.lookAtX, vals.lookAtY, vals.lookAtZ));
  ...
  return (
    <group ref={robotGroupRef}>
          ...
          <group rotation={[0, Math.PI, 0]}>
            ...
            <group position={[0, 0, 1.05]}>
              ...
              <group ref={chipRef} position={[0, 0, -0.06]}>
                ...
                {/* Spinning holographic scanning ring */}
                <mesh ref={cpuRingRef} position={[0, 0, 0.04]}>
      ...
      {/* 3. CENTRAL MOTHERBOARD & POP-UP CPU (visible only when split) */}
      {explodedProgress > 0.05 && (
        <group position={[0, 0, 0]}>
          {/* Circular motherboard plate */}
          <mesh rotation={[Math.PI / 2, 0, 0]} position={[0, -0.02, 0]}>
            <cylinderGeometry args={[1.0, 1.0, 0.04, 32]} />
            <meshBasicMaterial color="#1a3d24" />
          </mesh>
          <mesh rotation={[Math.PI / 2, 0, 0]} position={[0, -0.02, 0]} scale={[outlineScale[0], 1, outlineScale[2]]}>
            <cylinderGeometry args={[1.0, 1.0, 0.04, 32]} />
            <meshBasicMaterial color="#141312" side={THREE.BackSide} />
          </mesh>

          {/* Solder wires / Circuit tracks on the motherboard */}
          <gridHelper args={[1.8, 8, '#3bf574', '#3bf574']} position={[0, 0.005, 0]}>
            <lineBasicMaterial attach="material" transparent opacity={0.3} />
          </gridHelper>

          {/* Rising CPU Chipset */}
          <group position={[0, 0.02 + animValuesRef.current.chipPopProgress * 0.45, 0]}>
            {/* The main chip block */}
            <mesh castShadow>
            ...
            <mesh scale={[outlineScale[0], 1.05, outlineScale[2]]}>
              <boxGeometry args={[0.42, 0.1, 0.42]} />
              <meshBasicMaterial color="#141312" side={THREE.BackSide} />
            </mesh>

            {/* Glowing gold processor core */}
            <mesh position={[0, 0.06, 0]}>
              <boxGeometry args={[0.2, 0.03, 0.2]} />
              <meshBasicMaterial color="#e9b44c" toneMapped={false} />
            </mesh>

            {/* Spinning cyber-ring scanner */}
            <mesh ref={centralCpuRingRef} position={[0, 0.07, 0]} rotation={[Math.PI / 2, 0, 0]}>
              <ringGeometry args={[0.28, 0.3, 32]} />
              <meshBasicMaterial color="#00f3ff" toneMapped={false} transparent opacity={0.8} />
            </mesh>
