  ...
  devtools(
    (set, get) => ({
      ...
      triggerScene: (sceneId) => {
        ...
        switch (sceneId) {
          case 'morning':
            // Turn on all lights at high brightness, set thermostat
            set((s) => ({
              placedObjects: s.placedObjects.map((o) => {
      ...
      executeVoiceCommand: (text: string): string => {
        ...
        // Good morning / night scenes
        if (/good morning/.test(lower)) {
          get().triggerScene('morning');
          return 'Good morning! Turning on the lights and setting the temperature.';
        }
        if (/good night|bedtime/.test(lower)) {
          get().triggerScene('night');
          return 'Good night! Turning off devices and locking the doors.';
        }
        if (/movie time|movie night/.test(lower)) {
          get().triggerScene('movie');
          return 'Enjoy the movie! Dimming the lights and turning on the TV.';
        }
        if (/away mode|leaving|i.m leaving/.test(lower)) {
          get().triggerScene('away');
          return 'Away mode activated. Securing your home.';
        }
        ...
        if (brightnessMatch) {
          ...
          set((s) => ({
            placedObjects: s.placedObjects.map((o) => {
              if (o.type === 'smart-bulb') {
                count++;
                return { ...o, alexaDeviceState: { ...o.alexaDeviceState, brightness: val } };
              }
              return o;
            }),
          }));
        ...
        // Toggle a specific device by name
        ...
        return "Sorry, I didn't understand that command. Try \"turn on the lights\" or \"good morning\".";
      },
        ...
        set((s) => {
          ...
          const placedObjects = s.placedObjects.map((o) => {
            ...
            if (o.type === 'motion-sensor' && tick % 30 === 0) {
              const wasDetected = ds.motionDetected;
              ds.motionDetected = Math.random() < 0.12;
              if (ds.motionDetected && !wasDetected) {
                // Motion detected — handled outside this map via a separate effect
              }
            }
            ...
            if (ds.powerConsumption !== undefined) {
              ds.powerConsumption = parseFloat(
                ((ds.powerConsumption ?? 0) * (0.95 + Math.random() * 0.1)).toFixed(1)
              );
            }
